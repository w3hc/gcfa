...
describe("gCFA", function () {
  ...
  describe("Interactions", function () {
    ...
    it("Should approve gCFA contract", async function () {
      ...
      expect(await eur.allowance(alice.address, cfa.address)).to.equal(parseEther('10000'))
    })
    ...
    it("Should withdraw EUR", async function () {
      ...
      // withdraw
      const currentBalance = ethers.utils.formatEther(ethers.BigNumber.from(await cfa.balanceOf(alice.address)))
    ...
    it("Should recover lost EUR", async function () {
      const { eur, cfa, alice, recovery } = await loadFixture(deployContractsFixture)
      expect(await eur.approve(cfa.address, parseEther('10000')))
      expect(await cfa.depositFor(alice.address, parseEther('100')))
      expect(await eur.allowance(alice.address, cfa.address)).to.equal(parseEther('9900'))
      ...
      expect(await cfa.balanceOf(recovery.address)).to.equal(parseEther('65595.700'))
    })

    it("Should recover lost CFA", async function () {
      const { eur, cfa, alice, recovery, rate } = await loadFixture(deployContractsFixture)
      expect(await eur.approve(cfa.address, parseEther('10000')))
      expect(await cfa.depositFor(alice.address, parseEther('100')))
      expect(await eur.allowance(alice.address, cfa.address)).to.equal(parseEther('9900'))
      expect(await eur.balanceOf(alice.address)).to.equal(parseEther('9900'))
