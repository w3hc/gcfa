...
async function main() {
  ...
  // deploy EUR
  const EUR = await ethers.getContractFactory("gEUR")
  ...
  const receipt2 = await ethers.provider.getTransactionReceipt(gcfa.deployTransaction.hash)
  console.log("\nBlock number:", msg(receipt2.blockNumber))

  try {
    console.log("\ngEUR contract Etherscan verification in progress...")
    await eur.deployTransaction.wait(6)
    await hre.run("verify:verify", { network: network.name, address: eur.address, constructorArguments: [], contract: "contracts/gEUR.sol:gEUR"})
    console.log("Etherscan verification done. ✅")

    console.log("\n gCFA contract Etherscan verification in progress...")
    await gcfa.deployTransaction.wait(6)
    await hre.run("verify:verify", { network: network.name, address: gcfa.address, constructorArguments: [eur.address], contract: "contracts/gCFA.sol:gCFA" })
    console.log("Etherscan verification done. ✅")
